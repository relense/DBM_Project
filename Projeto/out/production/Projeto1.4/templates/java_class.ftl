...
<#assign rqbool><#list required as rq>${rq.type} ${rq.name}<#sep>, </#list></#assign>
     ...
     */
    public ${name}(<#if attributes?has_content>int ${name?lower_case}Id,<#else>int ${name?lower_case}Id</#if><#list attributes as attribute>${attribute.type} ${attribute.name}<#sep>, </#list>) {
        this.${name?lower_case}Id = ${name?lower_case}Id;
        <#list attributes as attribute>
        this.${attribute.name} = ${attribute.name};
        </#list>
        <#list foreignKeys as fk>
        this.${fk} = new ${fk?cap_first}();
        </#list>
    }
    ...
    public static ArrayList<${name}> all() throws SQLException {
        ...
        return get${name}ResultSet("src/MyModels/${pkg}/tables.db","SELECT * FROM ${name}");
    }
    ...
    */
    public static ArrayList<${name}> where(String condition) throws SQLException {
        ...
        return get${name}ResultSet("src/MyModels/${pkg}/tables.db", "SELECT * FROM ${name} WHERE " + condition);
    }
    ...
    public ArrayList<${hk.name}> getAll${hk.name}(int fk_${name?lower_case}Id) throws SQLException{
        ...
        while (rs.next()) {
                ${hk.name} ${hk.name?lower_case} = new ${hk.name}(rs.getInt(rs.getMetaData().getColumnName(1)),
                ...
                <#elseif attribute.type == "int">rs.getInt(rs.getMetaData().getColumnName(${attribute?index + 2}))
                <#elseif attribute.type == "Date">rs.getDate(rs.getMetaData().getColumnName(${attribute?index + 2}))
                <#elseif attribute.type == "double">rs.getDouble(rs.getMetaData().getColumnName(${attribute?index + 2}))</#if></#compress><#sep>,
                 </#list>);
                </#if>
                <#if hk.foreignKeys?has_content>
    ...
    private static ArrayList<${name}> get${name}ResultSet(String filename, String query) throws SQLException {
        ...
        ArrayList<${name}> ${name?lower_case}List = new ArrayList<>();
               ...
               while (rs.next()) {
                      ${name} ${name?lower_case} = new ${name}(rs.getInt(rs.getMetaData().getColumnName(1))<#if attributes?has_content>, <#else>); </#if>
